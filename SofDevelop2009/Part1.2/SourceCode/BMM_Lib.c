...
//	printf("BMM_DisposeBlock::%s, blockNum=%d\n",finfo.filename,blockNum);
	for ( i=0; i< BF_BUFFER_SIZE; i++)
	{					/* find Block in memory */
		if ( Mem_info[i].Empty == FALSE )
		{		
//			printf("Mem_info[i].FileName=%s, %d\n-->%d %s<--\n\n",Mem_info[i].FileName,Mem_info[i].BlockNum 
//			,*((int *)Mem_info[i].Block),(char *)Mem_info[i].Block+4);
