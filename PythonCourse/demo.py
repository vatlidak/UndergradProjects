...
class queue_test(unittest.TestCase):
    ...
    def testPriority(self):
        ...
        out = [self.q.pop() for _ in xrange(10)]
        self.assertEqual(out, [3,4,9,8,10,7,1,5,6,2])
...
print "\nImplementation used:", pqueue.priority_queue
    ...
    class queue_test_speed(unittest.TestCase):
        ...
        def makeTest(action):
            def test(self):
                ...
                self.assert_(time_taken < fine_time_limit)
        ...
        def testSawtooth(self):
            ...
            for i in xrange(options.teeth):
                for j in xrange(per_tooth):
                    self.q.add(i,i%5)
                for j in xrange(per_tooth):
                    self.q.pop()
